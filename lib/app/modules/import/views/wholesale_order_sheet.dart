...
class WholesaleOrderSheet extends StatefulWidget {
  ...
  static Future<void> show({
    ...
    return Get.bottomSheet(
      WholesaleOrderSheet(product: product, shippingOptions: shippingOptions, countryFlag: countryFlag),
...
class _WholesaleOrderSheetState extends State<WholesaleOrderSheet> {
  ...
  void initState() {
    ...
    if (widget.shippingOptions.isNotEmpty) _shipping = widget.shippingOptions.first;
  }

  String _fmt(double v) {
    final s = v == v.roundToDouble() ? v.toStringAsFixed(0) : v.toStringAsFixed(0);
    return '${s.replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+$)'), (m) => '${m[1]} ')} FCFA';
  }

  double get _subtotal => (_tier?.unitPrice ?? 0) * _quantity;
  ...
  double get _shippingCost {
    ...
    switch (s.rateType) {
      case 'per_kg':
      case 'per_cbm':
        return s.rateAmount * _weightOrCbm;
      default:
        return s.rateAmount; // flat
    }
  }
          ...
          child: Column(
            ...
            children: [
                      ...
                      children: [
                        Row(children: [
                          ...
                          Container(
                            ...
                            child: const Text('COMMANDE EN GROS',
                                style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: Color(0xFFB45309), letterSpacing: 0.5)),
                          ),
                        ]),
                        const SizedBox(height: 4),
                        Text(p.name, maxLines: 2, overflow: TextOverflow.ellipsis,
                            style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppThemeSystem.getPrimaryTextColor(context))),
                      ],
                    ),
                  ),
                ],
              ...
              if (p.description != null && p.description!.isNotEmpty) ...[
                ...
                Text(p.description!, style: TextStyle(fontSize: 13, color: AppThemeSystem.getSecondaryTextColor(context))),
              ],
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Choisir un moyen de paiement', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
  ...
  Widget _imgPh() => Container(width: 56, height: 56, color: const Color(0xFFF1F2F5),
      child: const Icon(Icons.inventory_2_outlined, color: Color(0xFFB4BCC6)));

  Widget _label(BuildContext c, String t) => Text(t,
      style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppThemeSystem.getPrimaryTextColor(c)));
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(12),
          border: Border.all(
            color: selected ? AppThemeSystem.primaryColor : AppThemeSystem.getBorderColor(context),
            width: selected ? 2 : 1,
          ),
        ),
        child: Row(
          children: [
            Icon(selected ? Icons.radio_button_checked : Icons.radio_button_off,
                color: selected ? AppThemeSystem.primaryColor : AppThemeSystem.getSecondaryTextColor(context), size: 20),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(t.label, style: TextStyle(fontWeight: FontWeight.w600, color: AppThemeSystem.getPrimaryTextColor(context))),
                  const SizedBox(height: 2),
                  Text('Minimum ${t.minQuantity}', style: const TextStyle(fontSize: 12, color: Color(0xFFB45309), fontWeight: FontWeight.w600)),
                ],
              ),
            ),
            Text(t.formattedPrice, style: const TextStyle(fontWeight: FontWeight.w800, color: AppThemeSystem.primaryColor)),
          ],
        ),
      ),
    );
        ...
        Expanded(
          child: Center(
            child: Text('$_quantity',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppThemeSystem.getPrimaryTextColor(context))),
          ),
        ),
  ...
  Widget _shippingTile(ShippingOption s) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(_shipIcon(s.mode), color: selected ? AppThemeSystem.primaryColor : AppThemeSystem.getSecondaryTextColor(context)),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(s.modeLabel, style: TextStyle(fontWeight: FontWeight.w600, color: AppThemeSystem.getPrimaryTextColor(context))),
                  const SizedBox(height: 2),
                  Text(
                    '${s.formattedRate}${s.expeditionNote != null ? ' · ${s.expeditionNote}' : ''}',
                    maxLines: 2, overflow: TextOverflow.ellipsis,
                    style: TextStyle(fontSize: 11.5, color: AppThemeSystem.getSecondaryTextColor(context)),
                  ),
                ],
              ),
            ),
  ...
  Widget _weightRow(BuildContext context) {
    return TextField(
      ...
      onChanged: (v) => setState(() => _weightOrCbm = double.tryParse(v) ?? 0),
      decoration: InputDecoration(
        labelText: _needsWeight ? 'Poids estimé (kg)' : 'Volume estimé (CBM)',
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
        prefixIcon: Icon(_needsWeight ? Icons.scale_rounded : Icons.view_in_ar_rounded),
      ),
    );
  }
  ...
  Widget _summary(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          _sumRow(context, 'Produit (${_quantity} × ${_tier?.label ?? ''})', _fmt(_subtotal)),
          const SizedBox(height: 6),
          _sumRow(context, 'Expédition (${_shipping?.modeLabel ?? '—'})', _fmt(_shippingCost)),
  ...
  Widget _sumRow(BuildContext c, String l, String v, {bool bold = false}) => Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Expanded(child: Text(l, style: TextStyle(color: AppThemeSystem.getSecondaryTextColor(c), fontSize: bold ? 15 : 13, fontWeight: bold ? FontWeight.bold : FontWeight.normal))),
          Text(v, style: TextStyle(color: bold ? AppThemeSystem.primaryColor : AppThemeSystem.getPrimaryTextColor(c), fontWeight: FontWeight.bold, fontSize: bold ? 16 : 13)),
        ],
      );
  ...
  Future<void> _pay() async {
    ...
    if (_quantity < tier.minQuantity) {
      Get.snackbar('Quantité minimale', 'Minimum ${tier.minQuantity} pour ${tier.label}.', snackPosition: SnackPosition.BOTTOM);
      return;
    }
    if ((_needsWeight || _needsCbm) && _weightOrCbm <= 0) {
      Get.snackbar('Info requise', _needsWeight ? 'Indiquez le poids estimé (kg).' : 'Indiquez le volume estimé (CBM).', snackPosition: SnackPosition.BOTTOM);
      return;
    }

    final method = await PaymentMethodSelector.show(amount: _total, currency: 'XAF', amountLabel: 'Total à payer');
    if (method == null) return;

    final items = [
      {'product_id': widget.product.id, 'price_tier_id': tier.id, 'quantity': _quantity}
    ];
    final weight = _needsWeight ? _weightOrCbm : null;
    final cbm = _needsCbm ? _weightOrCbm : null;
  ...
  Future<void> _create(String mode, List<Map<String, dynamic>> items, int shippingId, double? weight, double? cbm,
    ...
    try {
      ...
      if (mode == 'kpay_direct') {
        _pollOrder(orderId);
        Get.snackbar('Commande créée', 'Validez le paiement sur votre téléphone (USSD).',
      ...
      if (GetPlatform.isAndroid || GetPlatform.isIOS) {
        await Get.to(() => PaymentWebView(paymentUrl: approvalUrl, paymentMethod: mode == 'paypal_direct' ? 'paypal' : 'stripe', paymentId: orderId ?? 0));
      } else {
